...
$totalStmt = $pdo->prepare("SELECT COUNT(*) FROM reports r JOIN bns_reports b ON r.id = b.report_id WHERE r.user_id = ?");
$totalStmt->execute([$userId]);
$totalReports = $totalStmt->fetchColumn();

$approvedStmt = $pdo->prepare("SELECT COUNT(*) FROM reports r JOIN bns_reports b ON r.id = b.report_id WHERE r.user_id = ? AND r.status = 'Approved' AND NOT EXISTS (SELECT 1 FROM report_archives a WHERE a.report_id = r.id AND a.is_archived = 1)");
$approvedStmt->execute([$userId]);
$approvedReports = $approvedStmt->fetchColumn();

$pendingStmt = $pdo->prepare("SELECT COUNT(*) FROM reports r JOIN bns_reports b ON r.id = b.report_id WHERE r.user_id = ? AND r.status = 'Pending' AND NOT EXISTS (SELECT 1 FROM report_archives a WHERE a.report_id = r.id AND a.is_archived = 1)");
$pendingStmt->execute([$userId]);
$pendingReports = $pendingStmt->fetchColumn();

$rejectedStmt = $pdo->prepare("SELECT COUNT(*) FROM reports r JOIN bns_reports b ON r.id = b.report_id WHERE r.user_id = ? AND r.status = 'Rejected' AND NOT EXISTS (SELECT 1 FROM report_archives a WHERE a.report_id = r.id AND a.is_archived = 1)");
