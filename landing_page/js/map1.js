...
// ===================== FULL CHART (SIMPLIFIED - ALL YEARS = LINE GRAPH) =====================
function renderFullChart() {
  ...
  // ==================== CITY TOTAL FROM DROPDOWN ====================
    ...
    fullChart = new Chart(ctx, {
      ...
      data: {
        labels: indicatorsToShow.map(ind => ind.dataset.label),
        datasets: [{
          label: `🏙️ CITY TOTAL (Sum of all barangays) - ${activeYear !== 'All' ? activeYear : 'All Years'}`,
          data: cityTotalData,
          backgroundColor: indicatorsToShow.map(ind => ind.dataset.color),
          borderColor: indicatorsToShow.map(ind => ind.dataset.color),
          borderWidth: 2,
  ...
  // ==================== ALL YEARS = LINE GRAPH (lahat ng cases) ====================
    ...
    // This covers: All Years + All Barangays OR All Years + Specific Barangay
    // Lahat ng may All Years = LINE GRAPH
    
    let labels, datasets;
    
    if (isAllBarangays) {
      // All Years + All Barangays: Show trend for each indicator (average across all barangays per year)
      labels = allYears;
      datasets = indicatorsToShow.map(indicator => {
        const data = allYears.map(year => {
          // Get average of all barangays for this year
      ...
      // All Years + Specific Barangay: Show trend for that barangay
